...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const exists = tables.some(t => (typeof t === 'string' ? t : t.tableName) === 'birthday_wish_logs');
        if (exists) return;

        await queryInterface.createTable('birthday_wish_logs', {
            id: {
        ...
        await queryInterface.addIndex('birthday_wish_logs', ['staff_id', 'wish_date'], {
            unique: true,
            name: 'birthday_wish_logs_staff_date_unique'
        });
    },

    down: async (queryInterface) => {
        await queryInterface.dropTable('birthday_wish_logs');
    }
};
