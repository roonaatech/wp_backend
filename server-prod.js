...
console.log("[PROD] DB_HOST from env:", process.env.DB_HOST);

// SSL Certificate Configuration
const SSL_CERT_PATH = '/apps/workpulse/certs';
const sslOptions = {
...
// Test route
app.get('/', (req, res) => {
    res.json({ message: 'Welcome to the WorkPulse Production API. Visit /api-docs for documentation.' });
});
...
db.sequelize.sync()
    .then(() => {
        console.log('[PROD] Synced db.');
        // Seed Email Templates
        require('./utils/seed_templates')();

        https.createServer(sslOptions, app).listen(PORT, () => {
            console.log(`[PROD] HTTPS Server is running on port ${PORT}.`);
            console.log(`[PROD] API Documentation available at https://api-workpulse.roonaa.in:${PORT}/api-docs`);
            // Keep process alive check
            setInterval(() => { }, 1000 * 60);
        });
    })
    .catch((err) => {
        console.log('[PROD] Failed to sync db: ' + err.message);
    });
